...
public class WaterReflection : MonoBehaviour
{
    ...
    public string arPlaneTag = "ARPlane";
    ...
    private void Update()
    {
        ...
        if (isReady && reflectionPlane != null)
            RenderReflection();
    }

    private void TryFindARPlane()
    {
        GameObject planeObj = GameObject.FindWithTag(arPlaneTag);
        if (planeObj != null)
        {
            reflectionPlane = planeObj.transform;
            Debug.Log("[WaterReflection] AR Plane 자동 연결됨: " + reflectionPlane.name);
        }
    }
